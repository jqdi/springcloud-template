...
数据库连接配置位于 `boot-starter-datasource` 的 `application-datasource.yml` 中，通常无需修改
...
code:
  generate:
    # 默认目标模块，支持: system,order,user,tool
    moduleName: order
    # 默认表名，多个表名用英文逗号分隔
...
```http
POST /code/generate?moduleName={moduleName}&tableName={tableName}
```
