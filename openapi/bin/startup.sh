...
# 启动脚本
# 前置准备：
# 1. 准备好服务器根目录'../应用根目录/template-openapi'
# 2. 将本脚本复制到'应用根目录/template-openapi/startup.sh'
# 3. 复制cicd/plugins/到'应用根目录/template-openapi/plugins/'
# 4. 将template-openapi.jar上传至'应用根目录/template-openapi/template-openapi.jar'


# 项目名，建议与项目名保持一致
PROJECT="springcloud-template"
# 应用名，建议与spring.application.name保持一致
MODULE="template-openapi"
